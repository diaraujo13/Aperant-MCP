...
fn detect_ides() -> Vec<Value> {
    ...
    ides.push(json!({ "id": "vscode", "name": "VS Code", "path": "code", "installed": vscode_installed }));
    ...
    let windsurf_installed = probe_binary("windsurf")
        || check_app_exists("/Applications/Windsurf.app");
...
fn detect_terminals() -> Vec<Value> {
    ...
    if cfg!(target_os = "macos") {
        ...
        terminals.push(json!({ "id": "terminal", "name": "Terminal", "path": "terminal", "installed": true }));
        ...
        for (id, name, bin) in [
            ...
            terminals.push(json!({ "id": id, "name": name, "path": bin, "installed": probe_binary(bin) }));
        }
    } else {
        terminals.push(json!({ "id": "windows-terminal", "name": "Windows Terminal", "path": "wt.exe", "installed": probe_binary("wt") }));
        terminals.push(json!({ "id": "cmd", "name": "Command Prompt", "path": "cmd.exe", "installed": true }));
...
pub async fn worktree_detect_tools() -> AppResult<IpcResult<Value>> {
    let result = tokio::task::spawn_blocking(|| json!({ "ides": detect_ides(), "terminals": detect_terminals() }))
        .await
...
#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_discard(task_id: String, _skip_status_change: Option<bool>) -> AppResult<IpcResult<Value>> {
    let result = tokio::task::spawn_blocking(move || -> (bool, Option<String>) {
        ...
        for p in store.projects() {
            ...
            if wt.is_dir() {
                ...
                if !ok { let _ = std::fs::remove_dir_all(&wt); }
    ...
    if result.0 {
        ...
        Ok(IpcResult { success: false, data: None, error: Some("worktree_not_found".to_string()) })
    }
}

#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_discard_orphaned(project_id: String, spec_name: String) -> AppResult<IpcResult<Value>> {
    let result = tokio::task::spawn_blocking(move || -> bool {
        ...
        let store = project::read_store_at(&store_path);
        let proj_path = store.projects().into_iter()
            .find(|p| p.get("id").and_then(|v| v.as_str()) == Some(&project_id))
            .and_then(|p| p.get("path").and_then(|v| v.as_str()).map(String::from));
        let Some(proj_path) = proj_path else { return false; };
        let wt = PathBuf::from(&proj_path).join(WORKTREE_SUBDIR).join(&spec_name);
        if wt.is_dir() {
            let ok = std::process::Command::new("git")
                .args(["worktree", "remove", "--force", &wt.to_string_lossy()])
                .current_dir(&proj_path)
...
#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_list(project_id: String, _include_stats: Option<bool>) -> AppResult<IpcResult<Value>> {
    let result = tokio::task::spawn_blocking(move || -> Value {
        let store_path = match project::store_path() {
            Ok(p) => p,
        ...
        let worktrees: Vec<Value> = std::fs::read_dir(&wt_dir).ok().into_iter().flatten()
...
#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_open_in_ide(worktree_path: String, ide: String, custom_path: Option<String>) -> AppResult<IpcResult<Value>> {
    tokio::task::spawn_blocking(move || {
        ...
        let _ = std::process::Command::new(cmd).arg(&worktree_path).stdout(Stdio::null()).stderr(Stdio::null()).spawn();
    })
    .await
    .map_err(|e| AppError::new("spawn_blocking_failed", e.to_string()))?;
...
#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_open_in_terminal(worktree_path: String, terminal: String, custom_path: Option<String>) -> AppResult<IpcResult<Value>> {
    tokio::task::spawn_blocking(move || {
        if cfg!(target_os = "macos") {
            ...
            let _ = std::process::Command::new("open").args(["-a", app, &worktree_path]).spawn();
        } else if cfg!(target_os = "linux") {
            ...
            let _ = std::process::Command::new(term).args(["--working-directory", &worktree_path]).spawn();
        } else {
            let _ = std::process::Command::new("cmd")
                .args(["/c", "start", "cmd", "/k", &format!("cd /d \"{}\"", worktree_path)])
                .spawn();
        }
    })
    .await
...
#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_merge(_task_id: String, _no_commit: Option<bool>) -> AppResult<IpcResult<Value>> {
    Ok(IpcResult { success: false, data: None, error: Some("worktree_merge_requires_python_backend".to_string()) })
}

#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_merge_preview(_task_id: String) -> AppResult<IpcResult<Value>> {
    Ok(IpcResult { success: false, data: None, error: Some("worktree_merge_preview_requires_python_backend".to_string()) })
}

#[tauri::command(rename_all = "camelCase")]
pub async fn worktree_create_pr(_task_id: String, _options: Option<Value>) -> AppResult<IpcResult<Value>> {
    Ok(IpcResult { success: false, data: None, error: Some("worktree_create_pr_requires_python_backend".to_string()) })
}
